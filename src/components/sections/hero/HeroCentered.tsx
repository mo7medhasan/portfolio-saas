...
export function HeroCentered({ content: c }: { content: HeroContent }) {
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-(--color-background,#fff)">
      ...
      <div className="relative z-10 mx-auto max-w-4xl px-6 py-24 flex flex-col items-center text-center gap-6">
        ...
        {c.imageUrl && (
          <div className="w-28 h-28 rounded-full overflow-hidden ring-4 ring-(--color-primary,#6C63FF)/20">
            <img src={c.imageUrl} alt="profile" className="w-full h-full object-cover" />
          </div>
        )}
        <h1 className="text-[clamp(2.8rem,6vw,5.5rem)] font-(--font-weight-heading,700) leading-[1.05] tracking-(--letter-spacing-heading,-0.02em)"
          style={{ fontFamily:"var(--font-heading,sans-serif)", color:"var(--color-heading,#111)" }}>
          {c.headline}
        </h1>
        {c.subheadline && (
          <p className="text-lg max-w-xl leading-(--line-height-body,1.7)" style={{ color:"var(--color-text-secondary,#555)" }}>
            {c.subheadline}
          </p>
        )}
        <div className="flex flex-wrap gap-3 justify-center mt-2">
          {c.ctaText && (
            <a href={c.ctaUrl || "#"} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-(--radius-md,8px) font-medium text-white transition-all hover:opacity-90 hover:-translate-y-0.5"
          ...
          {c.secondaryCtaText && (
            <a href={c.secondaryCtaUrl || "#"} className="inline-flex items-center gap-2 px-7 py-3.5 rounded-(--radius-md,8px) font-medium border border-(--color-border,#e5e5e5) transition-all hover:border-(--color-primary,#6C63FF)"
