...
function ServiceIcon({ icon }: { icon: string }) {
  return (
    <div className="w-12 h-12 rounded-(--radius-md,8px) flex items-center justify-center text-2xl flex-shrink-0"
...
export function ServicesSection({ content: c, variant }: Props) {
  ...
  if (variant === "list") return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-background,#fff)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6">
        {header}
        <div className="space-y-4">
          {c.items.map(item => (
            <div key={item.id}
              className="flex items-start gap-5 p-6 rounded-(--radius-lg,16px) border border-(--color-border,#e5e5e5) transition-all hover:border-(--color-primary,#6C63FF)/40 hover:-translate-y-0.5"
    ...
    <section className="py-(--section-padding-y,6rem) bg-(--color-surface,#f8f8f8)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-(--gap-cards,24px)">
          {c.items.map(item => (
            <div key={item.id}
              className="p-8 rounded-(--radius-lg,16px) border border-(--color-border,#e5e5e5) space-y-5 transition-all hover:border-(--color-primary,#6C63FF)/40 hover:-translate-y-1"
  ...
  return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-surface,#f8f8f8)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6">
        {header}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-(--gap-cards,24px)">
          {c.items.map(item => (
            <div key={item.id}
              className="p-6 rounded-(--radius-lg,16px) border border-(--color-border,#e5e5e5) space-y-4 transition-all hover:border-(--color-primary,#6C63FF)/40 hover:-translate-y-1"
              style={{ background:"var(--color-background,#fff)", boxShadow:"var(--shadow-card)" }}>
              <ServiceIcon icon={item.icon} />
              ...
              {item.price && (
                <p className="text-sm font-semibold pt-1 border-t border-(--color-border,#e5e5e5)"
