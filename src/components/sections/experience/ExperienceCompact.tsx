...
export function ExperienceCompact({ content: c }: { content: ExperienceContent }) {
  return (
    <section id="experience" className="py-[var(--section-padding-y,6rem)] bg-[var(--color-background,#fff)]">
      <div className="mx-auto max-w-2xl px-6">
        {c.heading && (
          <h2 className="text-[clamp(1.75rem,3vw,2.5rem)] font-[var(--font-weight-heading,700)] mb-10"
        ...
        <div className="space-y-4">
          {c.items.map((item) => (
            <div key={item.id} className="flex items-center justify-between py-3 border-b border-[var(--color-border,#e5e5e5)] last:border-0">
              <div>
