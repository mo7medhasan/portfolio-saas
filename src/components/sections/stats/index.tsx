...
function StatNumber({ item }: { item: StatsContent["items"][number] }) {
  return (
    <div className="text-center">
      <p className="text-[clamp(2rem,4vw,3rem)] font-(--font-weight-heading,700) leading-none"
...
export function StatsSection({ content: c, variant }: Props) {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {c.items.map((item) => (
              <div key={item.id} className="text-center">
                <p className="text-[clamp(2rem,4vw,3rem)] font-(--font-weight-heading,700) leading-none text-white"
  ...
  return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-surface,#f8f8f8)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6">
        {c.heading && (
          <h2 className="text-[clamp(1.75rem,3vw,2.5rem)] font-(--font-weight-heading,700) text-center mb-10"
