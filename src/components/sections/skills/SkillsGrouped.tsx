  ...
  return (
    <section id="skills" className="py-(--section-padding-y,6rem) bg-(--color-surface,#f8f8f8)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6">
        {c.heading && (
          <h2 className="text-[clamp(1.75rem,3vw,2.5rem)] font-(--font-weight-heading,700) mb-12 text-center"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Object.entries(grouped).map(([category, skills]) => (
            <div key={category} className="p-6 rounded-(--radius-lg,16px) border border-(--color-border,#e5e5e5)"
              ...
              <div className="flex flex-wrap gap-2">
                {skills.map((s) => (
                  <span key={s.id} className="px-3 py-1 text-sm rounded-(--radius-md,8px)"
