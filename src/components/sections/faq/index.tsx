...
function AccordionItem({ item, open, onToggle }: { item: FaqItem; open: boolean; onToggle: () => void }) {
  return (
    <div className="border-b border-(--color-border,#e5e5e5) last:border-0">
      <button onClick={onToggle} className="w-full flex items-center justify-between gap-4 py-5 text-left transition-colors hover:text-(--color-primary,#6C63FF)"
...
export function FaqSection({ content: c, variant }: Props) {
  ...
  return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-background,#fff)">
      <div className="mx-auto max-w-2xl px-6">
        <SectionHeader heading={c.heading} subheading={c.subheading} />
        <div className="rounded-(--radius-lg,16px) border border-(--color-border,#e5e5e5) px-6"
