...
export function CtaSection({ content: c, variant }: Props) {
  const ctaBtn = (dark: boolean) => (
    <a href={c.ctaUrl || "#contact"}
      className="inline-flex items-center gap-2 px-8 py-4 rounded-(--radius-md,8px) font-medium transition-all hover:opacity-90 hover:-translate-y-0.5"
  ...
  if (variant === "card") return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-surface,#f8f8f8)">
      <div className="mx-auto max-w-lg px-6">
        <div className="p-10 rounded-2xl text-center space-y-5 border border-(--color-border,#e5e5e5)"
          style={{ background:"var(--color-background,#fff)", boxShadow:"var(--shadow-card)" }}>
          <h2 className="text-2xl font-(--font-weight-heading,700)"
  ...
  if (variant === "minimal") return (
    <section className="py-(--section-padding-y,6rem) bg-(--color-background,#fff)">
      <div className="mx-auto max-w-(--container-max-width,1200px) px-6 flex flex-col sm:flex-row items-center justify-between gap-6">
        <div>
          <h2 className="text-xl font-(--font-weight-heading,700)"
            style={{ fontFamily:"var(--font-heading,sans-serif)", color:"var(--color-heading,#111)" }}>{c.heading}</h2>
  ...
  // default: banner
  return (
    <section className="py-(--section-padding-y,6rem)" style={{ background:"var(--color-primary,#6C63FF)" }}>
      <div className="mx-auto max-w-2xl px-6 text-center space-y-6">
        <h2 className="text-[clamp(1.75rem,3.5vw,3rem)] font-(--font-weight-heading,700) text-white"
