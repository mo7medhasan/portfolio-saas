  ...
  return (
    ...
    >
      ...
      {c.bgType === "image" && c.bgValue && (
        ...
        >
          <div className="absolute inset-0 bg-[var(--color-background,#fff)]/80" />
        </div>
      )}
      ...
      <div
        className={[
          ...
          "max-w-[var(--container-max-width,1200px)]",
          isSplit
            ? layout === "split-right"
      ...
      >
        ...
        <div className={isSplit ? "flex-1 space-y-6" : "max-w-3xl space-y-6"}>
          {/* Eyebrow */}
          <span className="inline-block text-xs font-semibold tracking-[0.2em] uppercase text-[var(--color-primary,#6C63FF)] opacity-80">
            {variant === "agency" ? "Creative Agency" : "Portfolio"}
          </span>

          <h1
            className="text-[clamp(2.5rem,6vw,5rem)] leading-[1.05] font-[var(--font-weight-heading,700)] tracking-[var(--letter-spacing-heading,-0.02em)]"
            style={{
          ...
          {c.subheadline && (
            <p
              className="text-lg leading-[var(--line-height-body,1.7)] max-w-xl"
          ...
          <div className={`flex gap-4 flex-wrap ${!isSplit ? "justify-center" : ""}`}>
            {c.ctaText && (
              <a
                ...
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-[var(--radius-md,8px)] font-medium text-white transition-all duration-[var(--transition-speed,0.2s)] hover:opacity-90 hover:-translate-y-0.5 active:translate-y-0"
            ...
            <a
              href="#portfolio"
              className="inline-flex items-center gap-2 px-7 py-3.5 rounded-[var(--radius-md,8px)] font-medium border border-[var(--color-border,#e5e5e5)] transition-all duration-[var(--transition-speed,0.2s)] hover:border-[var(--color-primary,#6C63FF)] hover:text-[var(--color-primary,#6C63FF)]"
        ...
        {isSplit && c.imageUrl && (
          <div className="flex-1 flex justify-center">
            <div className="relative w-72 h-72 md:w-96 md:h-96">
              {/* Ring decoration */}
              <div
                className="absolute inset-0 rounded-full border-2 border-[var(--color-primary,#6C63FF)] opacity-20 scale-110"
              />
              <img
                src={c.imageUrl}
                alt="hero"
                className="relative z-10 w-full h-full object-cover rounded-full shadow-[var(--shadow-card)]"
              />
            </div>
          </div>
        )}
        ...
        {!isSplit && !isFullscreen && (
          <div className="flex gap-8 mt-4 pt-8 border-t border-[var(--color-border,#e5e5e5)] w-full max-w-sm justify-center">
            {[
