  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background:"rgba(0,0,0,.4)" }}
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-(--color-border,#e5e5e5) flex flex-col max-h-[90vh]"
          ...
          <div className="flex gap-1 p-1 rounded-xl" style={{ background:"var(--color-surface,#f8f8f8)" }}>
            {(["content","design"] as Tab[]).map(t=>(
              <button key={t} onClick={()=>setTab(t)}
                className={`px-4 py-1.5 text-sm font-medium rounded-lg transition-all ${tab===t ? "bg-(--color-background,#fff) shadow-sm" : "opacity-60 hover:opacity-100"}`}
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-(--color-border,#e5e5e5) flex items-center justify-between">
          {error && <p className="text-sm text-red-500">{error}</p>}
          <div className="flex gap-3 ml-auto">
            <button onClick={onClose}
              className="px-4 py-2.5 text-sm rounded-(--radius-md,8px) border border-(--color-border,#e5e5e5) hover:border-(--color-primary,#6C63FF) transition-colors"
              style={{ color:"var(--color-text-primary,#111)" }}>إلغاء</button>
            <button onClick={save} disabled={saving}
              className="px-6 py-2.5 text-sm font-medium rounded-(--radius-md,8px) text-white disabled:opacity-50"
