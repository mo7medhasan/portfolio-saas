  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background:"rgba(0,0,0,.4)" }}
      ...
      <div className="w-full max-w-2xl rounded-2xl border border-[var(--color-border,#e5e5e5)] flex flex-col max-h-[90vh]"
          ...
          <div className="flex items-center gap-2">
            {steps.map((s,i)=>(
              <div key={s.id} className="flex items-center gap-2">
                ...
                <button
                  className={`text-sm font-medium transition-colors ${step===s.id ? "text-[var(--color-primary,#6C63FF)]" : i<stepIdx ? "hover:text-[var(--color-primary,#6C63FF)]" : "opacity-40"}`}
        ...
        {/* Footer */}
        {step!=="type" && (
          <div className="px-6 py-4 border-t border-[var(--color-border,#e5e5e5)] flex items-center justify-between gap-3">
            {error && <p className="text-sm text-red-500 flex-1">{error}</p>}
            <div className="flex gap-3 ml-auto">
              {step==="variant" && (
                <button onClick={()=>setStep("content")}
                  className="px-5 py-2.5 rounded-[var(--radius-md,8px)] text-sm font-medium text-white hover:opacity-90"
              ...
              {step==="content" && (
                <button onClick={save} disabled={saving}
                  className="px-6 py-2.5 rounded-[var(--radius-md,8px)] text-sm font-medium text-white hover:opacity-90 disabled:opacity-50"
                  style={{ background:"var(--color-primary,#6C63FF)" }}>
